  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: DropdownButton<String>(
        value: selectedNationality, // Display selected nationality
        isExpanded: true,
        icon: Icon(Icons.arrow_drop_down, color: AppColors.orange),
        iconSize: 24.sp,
        hint: Text("nationality".tr()), // Hint text when no value is selected
        elevation: 16,
        ...
        items: widget.countries.map<DropdownMenuItem<String>>((AvailableCountry country) {
          return DropdownMenuItem<String>(
            value: country.name,
            child: Text(country.name ?? 'Unknown Country'), // Display country name or fallback to "Unknown Country"
          );
        }).toList(),
      ),
    );
